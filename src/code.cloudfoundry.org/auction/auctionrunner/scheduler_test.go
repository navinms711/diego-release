...
var _ = Describe("Scheduler", func() {
	...
	Context("freshness budget (Top-N with per-cell cap)", func() {
		var freshCell1, freshCell2, oldCell *repfakes.FakeSimClient

		BeforeEach(func() {
			freshCell1 = &repfakes.FakeSimClient{}
			freshCell2 = &repfakes.FakeSimClient{}
			oldCell = &repfakes.FakeSimClient{}

			clients["fresh-cell-1"] = freshCell1
			clients["fresh-cell-2"] = freshCell2
			clients["old-cell"] = oldCell
		})

		It("applies freshness bonus to the top N cells and removes it after K placements", func() {
			// 2 fresh cells (started recently), 1 old cell
			// evacuatingCount=2 means both fresh cells get the bonus
			// freshLRPsPerCell=5 means each fresh cell gets bonus for first 5 LRPs
			freshLRPsPerCell := 5
			now := time.Now()
			...
			// plus extras that should spread normally
			var lrpAuctions []auctiontypes.LRPAuction
			for i := 0; i < 15; i++ {
				lrpAuctions = append(lrpAuctions,
					BuildLRPAuction("pg-fresh-test", "domain", i, linuxRootFSURL, 10, 10, 10, clock.Now(), nil, []string{}))
			}
			...
			Expect(results.SuccessfulLRPs).To(HaveLen(15))

			freshCount := 0
			oldCount := 0
			for _, lrp := range results.SuccessfulLRPs {
				if lrp.Winner == "fresh-cell-1" || lrp.Winner == "fresh-cell-2" {
					freshCount++
				} else if lrp.Winner == "old-cell" {
					oldCount++
				}
			}

			By("some LRPs should land on the old cell after fresh cells' budgets are exhausted")
			Expect(oldCount).To(BeNumerically(">", 0),
				"After fresh cells' budgets of %d each are exhausted, remaining LRPs should spread to old cells",
				freshLRPsPerCell)
		})

		It("does not apply freshness when evacuatingCount is 0", func() {
			now := time.Now()
			zones["zone-a"] = auctionrunner.Zone{
				auctionrunner.NewCell(logger, "fresh-cell-1", freshCell1,
					BuildCellStateWithStartTime("cellID-1", 0, "zone-a", 1000, 1000, 100, false, 0,
			...
			for i := 0; i < 6; i++ {
				lrpAuctions = append(lrpAuctions,
					BuildLRPAuction("pg-no-evac", "domain", i, linuxRootFSURL, 10, 10, 10, clock.Now(), nil, []string{}))
			}
			...
			By("without evacuating cells, LRPs should spread roughly equally")
			Expect(freshCount).To(BeNumerically(">", 0))
			Expect(oldCount).To(BeNumerically(">", 0))
		})

		It("caps per-cell placements at freshLRPsPerCell with single fresh cell", func() {
			freshLRPsPerCell := 5
			now := time.Now()
			zones["zone-a"] = auctionrunner.Zone{
				auctionrunner.NewCell(logger, "fresh-cell-1", freshCell1,
			...
			for i := 0; i < 10; i++ {
				lrpAuctions = append(lrpAuctions,
					BuildLRPAuction("pg-cap-test", "domain", i, linuxRootFSURL, 10, 10, 10, clock.Now(), nil, []string{}))
			}
			...
			oldCount := 0
			for _, lrp := range results.SuccessfulLRPs {
				if lrp.Winner == "old-cell" {
					oldCount++
				}
			}

			By("after freshLRPsPerCell placements, remaining LRPs should go to old cell")
			Expect(oldCount).To(BeNumerically(">", 0),
				"Expected some LRPs on old-cell after fresh cell budget of %d exhausted",
				freshLRPsPerCell)
		})

		It("applies unlimited freshness bonus when freshLRPsPerCell is 0", func() {
			now := time.Now()
			zones["zone-a"] = auctionrunner.Zone{
				auctionrunner.NewCell(logger, "fresh-cell-1", freshCell1,
					BuildCellStateWithStartTime("cellID-1", 0, "zone-a", 1000, 1000, 100, false, 0,
			...
			var lrpAuctions []auctiontypes.LRPAuction
			for i := 0; i < 10; i++ {
				pgGuid := fmt.Sprintf("pg-unlimited-%d", i)
				lrpAuctions = append(lrpAuctions,
					BuildLRPAuction(pgGuid, "domain", 0, linuxRootFSURL, 10, 10, 10, clock.Now(), nil, []string{}))
			}
			...
			By("with unlimited budget, all LRPs should go to the fresh cell")
			Expect(freshCount).To(Equal(10))
		})
	})
})
