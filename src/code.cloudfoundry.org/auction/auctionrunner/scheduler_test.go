...
var _ = Describe("Scheduler", func() {
	...
	BeforeEach(func() {
		...
		scheduler = auctionrunner.NewScheduler(workPool, map[string]auctionrunner.Zone{}, clock, logger, 0.0, 0.0, 0, 0, 0.0, 0, nil)
	})
	...
	Context("when cells have inflight container creations", func() {
		...
		Context("and the current auction would exceed the maximum inflight container creations", func() {
			...
			BeforeEach(func() {
				...
				scheduler = auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, startingContainerCountMaximum, 0, 0.0, 0, nil)
	...
	Describe("handling start auctions", func() {
			...
			Context("with a new LRP only supported in one of many zones", func() {
				...
				Context("when it picks a winner", func() {
					BeforeEach(func() {
						...
						s := auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, 0, 0, 0.0, 0, nil)
		...
		Context("filtering on placement tags", func() {
			...
			JustBeforeEach(func() {
				...
				scheduler = auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, defaultStartingContainerCountMaximum, 0, 0.0, 0, nil)
			})
	...
	Describe("ordering work", func() {
		...
		JustBeforeEach(func() {
			...
			scheduler := auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, 0, 0, 0.0, 0, nil)
			results = scheduler.Schedule(auctionRequest)
		})
...
var _ = Describe("Scheduler freshness", func() {
	...
	It("does not apply the bonus when no cells are evacuating, even if a cell is recently started", func() {
		scheduler := auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, 0, 0, 50.0, 0, nil)
	...
	It("prefers the recently-started cell over one with better raw resources when cells are evacuating", func() {
		scheduler := auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, 0, 0, 50.0, 1, nil)
		auction := BuildLRPAuction("pg-1", "domain", 0, linuxRootFSURL, 5, 0, 0, clock.Now(), nil, []string{})
	...
	It("caps freshness placements at fresh_lrps_per_cell, reverting to normal scoring once exhausted", func() {
		scheduler := auctionrunner.NewScheduler(workPool, zones, clock, logger, 0.0, 0.0, 0, 1, 50.0, 1, nil)
