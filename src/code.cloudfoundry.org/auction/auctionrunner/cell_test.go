...
var _ = Describe("Cell", func() {
	...
	Describe("ScoreForLRP", func() {
		It("factors in memory usage", func() {
			...
			bigScore, err := emptyCell.ScoreForLRP(bigInstance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
			smallScore, err := emptyCell.ScoreForLRP(smallInstance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())

			Expect(smallScore).To(BeNumerically("<", bigScore))

			By("factoring in the relative emptiness of Cells")
			emptyScore, err := emptyCell.ScoreForLRP(smallInstance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
			score, err := cell.ScoreForLRP(smallInstance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
		...
		Context("when the cell has proxies enabled", func() {
			...
			Context("when there is enough memory for the cell and the proxy", func() {
				...
				It("succeeds placing the lrp", func() {
					score, err := proxiedCell.ScoreForLRP(lrp, 0.0, 0.0, 0.0, false, 0.0)
					Expect(err).NotTo(HaveOccurred())
		...
		It("factors in container usage", func() {
			...
			bigScore, err := bigCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
			smallScore, err := smallCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
		...
		Context("Weighted Bin Pack First Fit algorithm", func() {
			...
			It("factors in Bin Pack First Fit algorithm when a weight is provided", func() {
				...
				cellZeroScore, err := cellZero.ScoreForLRP(instance, 0.0, binPackFirstFitWeight, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
				cellOneScore, err := cellOne.ScoreForLRP(instance, 0.0, binPackFirstFitWeight, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("Starting Containers", func() {
			...
			It("factors in starting containers when a weight is provided", func() {
				...
				busyScore, err := busyCell.ScoreForLRP(instance, startingContainerWeight, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
				boredScore, err := boredCell.ScoreForLRP(instance, startingContainerWeight, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
				...
				smallerWeightScore, err := smallerWeightCell.ScoreForLRP(instance, startingContainerWeight-0.1, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
		...
		It("factors in process-guids that are already present", func() {
			...
			twoMatchesScore, err := cell.ScoreForLRP(instanceWithTwoMatches, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
			oneMatchesScore, err := cell.ScoreForLRP(instanceWithOneMatch, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
			noMatchesScore, err := cell.ScoreForLRP(instanceWithNoMatches, 0.0, 0.0, 0.0, false, 0.0)
			Expect(err).NotTo(HaveOccurred())
		...
		It("applies droplet locality bonus when cell has LRP droplet in cache and weight > 0", func() {
			dropletHash := "abc123def456"
			stateWithCache := BuildCellState("cell-with-cache", 0, "zone", 100, 100, 10, false, 0, linuxOnlyRootFSProviders, nil, []string{}, []string{}, []string{}, 0)
			stateWithCache.CachedDropletHashes = []string{dropletHash, "otherhash"}
			cellWithCache := auctionrunner.NewCell(logger, "cell-with-cache", client, stateWithCache)
			stateWithoutCache := BuildCellState("cell-without-cache", 0, "zone", 100, 100, 10, false, 0, linuxOnlyRootFSProviders, nil, []string{}, []string{}, []string{}, 0)
			cellWithoutCache := auctionrunner.NewCell(logger, "cell-without-cache", client, stateWithoutCache)

			lrpWithDroplet := BuildLRP("pg-droplet", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
			lrpWithDroplet.DropletCacheKeyHash = dropletHash
			lrpWithoutDroplet := BuildLRP("pg-droplet", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
			dropletLocalityWeight := 100.0

			scoreWithCache, err := cellWithCache.ScoreForLRP(lrpWithDroplet, 0.0, 0.0, 0.0, false, dropletLocalityWeight)
			Expect(err).NotTo(HaveOccurred())
			scoreWithoutCache, err := cellWithoutCache.ScoreForLRP(lrpWithDroplet, 0.0, 0.0, 0.0, false, dropletLocalityWeight)
			Expect(err).NotTo(HaveOccurred())
			Expect(scoreWithCache).To(BeNumerically("<", scoreWithoutCache), "cell with droplet in cache should score better (lower)")

			scoreNoHashOnLrp, err := cellWithCache.ScoreForLRP(lrpWithoutDroplet, 0.0, 0.0, 0.0, false, dropletLocalityWeight)
			Expect(err).NotTo(HaveOccurred())
			Expect(scoreNoHashOnLrp).To(Equal(scoreWithoutCache), "when LRP has no droplet hash, no locality bonus")
		})
		...
		Context("when the LRP does not fit", func() {
			Context("because of memory constraints", func() {
				It("should error", func() {
					...
					score, err := cell.ScoreForLRP(massiveMemoryInstance, 0.0, 0.0, 0.0, false, 0.0)
					Expect(score).To(BeZero())
			...
			Context("because of disk constraints", func() {
				It("should error", func() {
					...
					score, err := cell.ScoreForLRP(massiveDiskInstance, 0.0, 0.0, 0.0, false, 0.0)
					Expect(score).To(BeZero())
			...
			Context("because of container constraints", func() {
				It("should error", func() {
					...
					score, err := zeroCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
					Expect(score).To(BeZero())
		...
		Context("freshness weight (Top-N bonus for cells marked as fresh)", func() {
			It("applies full bonus to cells marked as fresh", func() {
				instance := BuildLRP("pg-new", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
				freshnessWeight := 100.0

				// Create a cell (startTime doesn't matter for direct ScoreForLRP call,
				// what matters is the isFreshCell flag passed by the Scheduler)
				freshCellState := BuildCellStateWithStartTime(
					"cellID", 0, "the-zone", 100, 200, 50, false, 0, linuxOnlyRootFSProviders,
					nil, []string{}, []string{}, []string{}, 0, time.Now(), nil,
				)
				freshCell := auctionrunner.NewCell(logger, "fresh-cell", client, freshCellState)

				freshScore, err := freshCell.ScoreForLRP(instance, 0.0, 0.0, freshnessWeight, true, 0.0)
				Expect(err).NotTo(HaveOccurred())

				By("cells marked as fresh should get a negative score (bonus)")
				Expect(freshScore).To(BeNumerically("<", 0))
				Expect(freshScore).To(BeNumerically("~", -freshnessWeight, 1.0))
			})

			It("applies no bonus to cells not marked as fresh", func() {
				instance := BuildLRP("pg-new", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
				freshnessWeight := 100.0

				// Create a cell
				oldCellState := BuildCellStateWithStartTime(
					"cellID", 0, "the-zone", 100, 200, 50, false, 0, linuxOnlyRootFSProviders,
					nil, []string{}, []string{}, []string{}, 0, time.Now().Add(-10*time.Minute), nil,
				)
				oldCell := auctionrunner.NewCell(logger, "old-cell", client, oldCellState)

				scoreNotFresh, err := oldCell.ScoreForLRP(instance, 0.0, 0.0, freshnessWeight, false, 0.0)
				Expect(err).NotTo(HaveOccurred())

				scoreNoWeight, err := oldCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())

				By("cells not marked as fresh should get no bonus")
				Expect(scoreNotFresh).To(Equal(scoreNoWeight))
			})

			It("does not apply bonus when freshnessWeight is 0 even if cell is marked fresh", func() {
				instance := BuildLRP("pg-new", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
				...
				scoreWithFreshFlag, err := freshCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, true, 0.0)
				Expect(err).NotTo(HaveOccurred())

				scoreWithoutFreshFlag, err := freshCell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())

				Expect(scoreWithFreshFlag).To(Equal(scoreWithoutFreshFlag))
			})

			It("prefers fresh cells over non-fresh cells even when fresh cell has more LRPs", func() {
				instance := BuildLRP("pg-new", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})
				freshnessWeight := 200.0

				// Empty but not-fresh cell
				oldEmptyCellState := BuildCellStateWithStartTime(
					"cellID", 0, "the-zone", 100, 200, 50, false, 0, linuxOnlyRootFSProviders,
					nil, []string{}, []string{}, []string{}, 0, time.Now().Add(-10*time.Minute), nil,
				)
				oldEmptyCell := auctionrunner.NewCell(logger, "old-empty-cell", client, oldEmptyCellState)

				// Fresh cell with some LRPs
				freshCellWithLRPs := BuildCellStateWithStartTime(
					"cellID", 0, "the-zone", 100, 200, 50, false, 0, linuxOnlyRootFSProviders,
					[]rep.LRP{*BuildLRP("pg-1", "domain", 0, linuxRootFSURL, 10, 10, 10, []string{})},
					[]string{}, []string{}, []string{}, 0, time.Now(), nil,
				)
				freshBusyCell := auctionrunner.NewCell(logger, "fresh-busy-cell", client, freshCellWithLRPs)

				oldEmptyScore, err := oldEmptyCell.ScoreForLRP(instance, 0.0, 0.0, freshnessWeight, false, 0.0)
				Expect(err).NotTo(HaveOccurred())

				freshBusyScore, err := freshBusyCell.ScoreForLRP(instance, 0.0, 0.0, freshnessWeight, true, 0.0)
				Expect(err).NotTo(HaveOccurred())

				By("fresh cells should be preferred over non-fresh cells due to freshness bonus")
				Expect(freshBusyScore).To(BeNumerically("<", oldEmptyScore))
			})
		})
	})
	...
	Describe("ReserveLRP", func() {
		Context("when there is room for the LRP", func() {
			It("should register its resources usage and keep it in mind when handling future requests", func() {
				...
				initialScore, err := cell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())

				Expect(cell.ReserveLRP(instanceToAdd)).To(Succeed())

				subsequentScore, err := cell.ScoreForLRP(instance, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
			...
			It("should register the LRP and keep it in mind when handling future requests", func() {
				...
				initialScoreForInstanceWithMatchingProcessGuid, err := cell.ScoreForLRP(instanceWithMatchingProcessGuid, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
				...
				subsequentScoreForInstanceWithMatchingProcessGuid, err := cell.ScoreForLRP(instanceWithMatchingProcessGuid, 0.0, 0.0, 0.0, false, 0.0)
				Expect(err).NotTo(HaveOccurred())
